...
class RobotGUI:
    ...
    # ------------------------------------------------- Meal -----------------------------------------------------------------------------
        ...
        if cmd == "button1_on" and self.task_mode == "meal":
            self.start_sequence()
        ...
        elif self.chewing_start_num >= 1 and self.food_num == 1 and self.task_mode == "meal":
            self.start_next("arm", "move_to_food2_start_main")
            self.food_num = 2

        elif cmd == "move_to_food2_finish_arm" and self.chewing_end_num >= 1 and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "move_from_food2_to_mouth_start_main")

        elif cmd == "move_from_food2_to_mouth_finish_arm" and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "chewing_sound")
            self.countdown(3)
        ...
        elif self.chewing_start_num >= 2 and self.chewing_end_num >= 1 and self.food_num == 2 and self.task_mode == "meal":
            self.start_next("arm", "move_to_food3_start_main")
            self.food_num = 3

        elif cmd == "move_to_food3_finish_arm" and self.chewing_end_num >= 2 and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "move_from_food3_to_mouth_start_main")

        elif cmd == "move_from_food3_to_mouth_finish_arm" and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "chewing_sound")
            self.countdown(3)
        ...
        elif self.chewing_start_num >= 3 and self.chewing_end_num >= 2 and self.food_num == 3 and self.task_mode == "meal":
            self.start_next("arm", "move_to_food4_start_main")
            self.food_num = 4

        elif cmd == "move_to_food4_finish_arm" and self.chewing_end_num >= 3 and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "move_from_food4_to_mouth_start_main")

        elif cmd == "move_from_food4_to_mouth_finish_arm" and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "chewing_sound")
            self.countdown(3)
        ...
        elif self.chewing_start_num >= 4 and self.chewing_end_num >= 3 and self.food_num == 4 and self.task_mode == "meal":
            self.start_next("arm", "move_to_food5_start_main")
            self.food_num = 5

        elif cmd == "move_to_food5_finish_arm" and self.chewing_end_num >= 4 and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "move_from_food5_to_mouth_start_main")

        elif cmd == "move_from_food5_to_mouth_finish_arm" and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "chewing_sound")
            self.countdown(3)
        ...
        elif self.chewing_start_num >= 5 and self.chewing_end_num >= 4 and self.food_num == 5 and self.task_mode == "meal":
            self.start_next("arm", "move_to_food6_start_main")
            self.food_num = 6

        elif cmd == "move_to_food6_finish_arm" and self.chewing_end_num >= 5 and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "move_from_food6_to_mouth_start_main")

        elif cmd == "move_from_food6_to_mouth_finish_arm" and self.task_mode == "meal":
            self.mark_done("arm")
            self.countdown(0.01)
            self.start_next("arm", "chewing_sound")
            self.countdown(3)
        ...
        elif self.chewing_start_num >= 6 and self.chewing_end_num >= 5 and self.food_num == 6 and self.task_mode == "meal":
            self.start_next("arm", "move_to_fix_start_main")
...
########################## Brush start #############################
        ...
        elif cmd == "button3_on" and self.task_mode == "brush":
        ...
        elif cmd == "button1_off" and self.task_mode == "brush": 
            self.mark_done("arm")
            self.start_next("arm", "dock_off_start_sound")
            self.countdown(0.01)
            self.start_next("mobile", "dock_off_start_main")           

        elif cmd == "dock_off_finish_mobile" and self.task_mode == "brush": 
            self.mark_done("mobile")
            self.countdown(0.01)
        ...
        elif cmd == "move_to_charger_finish_mobile" and self.task_mode == "brush": 
            self.mark_done("mobile")
            self.countdown(0.01)
...
##################### Reposition start #########################
        ...
        elif cmd == "move_from_default_to_cushion1_finish_arm" and self.task_mode == "reposition":    
            self.countdown(0.01)
        ...
        elif cmd == "button1_off" and self.task_mode == "reposition": 
            self.mark_done("arm")
            self.start_next("arm", "dock_off_start_sound")
            self.countdown(0.01)
            self.start_next("mobile", "dock_off_start_main")           

        elif cmd == "dock_off_finish_mobile" and self.task_mode == "reposition": 
            self.mark_done("mobile")
            self.countdown(0.01)
            self.start_next("arm", "dock_off_finish_sound")
            self.countdown(0.01)
        ...
        elif cmd == "move_to_charger_finish_mobile" and self.task_mode == "reposition": 
            self.mark_done("mobile")
            self.countdown(0.01)
            self.start_next("arm", "move_to_charger_finish_sound")
            self.task_mode = "fall"
...
################### Fall prevention start ######################
        ...
        elif cmd == "fall_first_outside" and self.task_mode == "fall":
            print("fall_first_outside success")
            self.vision_fall = 1

        elif cmd == "fall_second_outside" and self.task_mode == "fall":
            print("fall_second_outside success")
            self.vision_fall = 2

        elif cmd == "fall_third_outside" and self.task_mode == "fall":
            print("fall_third_outside success")
            self.vision_fall = 3

        elif self.vision_fall == 1 and self.task_mode == "fall":
            self.start_next("arm", "move_to_bed_start_sound")
            self.arduino_UI.write(b"button1_on_pc\n")
            subprocess.run([os.path.expanduser("~/ccmedia/ccmedia_R16")], check=True)
            self.countdown(0.01)
            self.start_next("mobile", "move_to_bed_first_gap_start_main")

        elif cmd == "move_to_bed_first_gap_finish_mobile" and self.task_mode == "fall":
            self.mark_done("mobile")
            self.countdown(0.01)
            self.start_next("arm", "prevent_fall_posture_start_main")
            self.countdown(0.01)
        ...
        elif cmd == "prevent_fall_posture_finish_arm" and self.task_mode == "fall":
            self.mark_done("mobile")
            self.countdown(0.01)
            self.start_next("arm", "warning_sound")

        elif cmd == "button1_off" and self.task_mode == "fall":
            self.mark_done("arm")
            self.start_next("arm", "dock_off_start_sound")
            self.countdown(0.01)
            self.start_next("mobile", "dock_off_start_main")           

        elif cmd == "dock_off_finish_mobile" and self.task_mode == "fall":
            self.mark_done("mobile")
            self.countdown(0.01)
            self.start_next("arm", "dock_off_finish_sound")
            self.countdown(0.01)
        ...
        elif cmd == "move_to_charger_finish_mobile" and self.task_mode == "fall":
            self.mark_done("mobile")
            self.countdown(0.01)
            self.start_next("arm", "move_to_charger_finish_sound")
            self.task_mode = "meal" 
